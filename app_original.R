...
ui <- fluidPage(
  tags$head(
    ...
    tags$style(HTML("
      ...
      * { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      #user_input::placeholder { color: #8888aa !important; font-size: 0.97rem !important; }
      ...
      #visitor_name::placeholder { color: #4a4a6a !important; }
  ...
  tags$script(HTML("
    ...
    Shiny.addCustomMessageHandler('animateLastAgent', function(msg) {
      setTimeout(function() {
        var rows = document.querySelectorAll('.msg-row.agent');
        if (!rows.length) return;
        var last = rows[rows.length - 1];
        last.style.opacity = '0';
        last.style.transform = 'translateY(10px)';
        last.style.transition = 'opacity 0.35s ease, transform 0.35s ease';
        setTimeout(function() {
...
server <- function(input, output, session) {
  ...
  chat$chat(paste0(
    ...
    "GENERAL RULES:\n",
    "- You ONLY answer questions about Ditiro Letsoalo. Nothing else.\n",
    "- If asked anything unrelated to Ditiro (e.g. general knowledge, other people, coding help, current events), politely decline and redirect. Example: I am only here to tell you about Ditiro! Feel free to ask me anything about him.\n",
    ...
    "- Never say 'according to his CV', 'based on his CV', 'his CV says' or anything like that. Speak naturally.\n",
    "- Be warm, professional and enthusiastic\n",
    "- If asked something not covered, encourage them to reach out to Ditiro directly\n",
    "- FORMATTING: When answering questions that have multiple points, use this exact style:\n",
    "  * Use a short bold heading followed by a newline\n",
    "  * Then use bullet points (- ) for each item\n",
    "  * Never use markdown ** for bold or ### for headings\n",
  ...
  observeEvent(input$start_chat, {
    ...
    if (nchar(name) > 0) visitor_name(name)
  })
  ...
  output$chat_history <- renderUI({
    ...
    if (is.null(visitor) || visitor == "") {
      return(
        div(class = "welcome-screen",
            div(style = "font-size:2.5rem;", "👋"),
            tags$h2("Welcome!"),
            tags$p("Before we start, what's your name?"),
            div(class = "name-input-wrap",
                ...
                actionButton("start_chat", "Let's go →")
            )
        )
      )
    }
    ...
    if (length(msgs) == 0 && !is_waiting) {
      greeting <- {
        ...
        if (h >= 5  && h < 12) paste0("Good morning, ", visitor, "! ☀️")
        else if (h >= 12 && h < 17) paste0("Good afternoon, ", visitor, "! 👋")
        else if (h >= 17 && h < 21) paste0("Good evening, ", visitor, "! 🌆")
        else paste0("Hey ", visitor, "! 🌙")
      }
      return(
        div(class = "empty-state",
            div(class = "icon", "💬"),
            tags$p(greeting),
            tags$p(style = "margin-top:6px; font-size:0.85rem;", "Ask me anything about Ditiro!"),
    ...
    bubbles <- lapply(msgs, function(m) {
      if (m$role == "user") {
        ...
        if (isTRUE(m$photo)) {
          ...
          # Format: bold headings ending in ":" and bullet points starting with "- "
          lines <- strsplit(m$text, "\n")[[1]]
          html_parts <- lapply(lines, function(line) {
            trimmed <- trimws(line)
            if (nchar(trimmed) == 0) {
              tags$br()
            } else if (startsWith(trimmed, "- ")) {
              tags$div(style = "display:flex; gap:8px; margin:3px 0;",
                       tags$span(style = "color:#a78bfa; margin-top:2px;", "•"),
                       tags$span(substring(trimmed, 3))
              )
            } else if (endsWith(trimmed, ":") && nchar(trimmed) < 60) {
              tags$div(style = "font-weight:700; color:#ffffff; margin-top:10px; margin-bottom:2px; font-size:0.93rem;", trimmed)
            } else {
              tags$div(style = "margin:2px 0;", trimmed)
            }
          })
          div(class = "msg-row agent",
              div(class = "msg-avatar agent", "✦"),
              div(class = "msg-bubble agent", html_parts)
          )
        }
      }
    })
